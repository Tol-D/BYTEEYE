...
class cfg_construction:
    ...
    def cfg_output(self,AA,n,log,edge_analysis):
        ...
        print(json_str)
        with open('/home/jenny/PycharmProjects/ByteEye/Test Result/new_scp_re{}.json'.format(n), 'w') as f:
            f.write(json_str)
...
def main():
    ...
    #recode the wrong edges
    # f = open('/home/jenny/PycharmProjects/ByteEye/Core/Analysis/wrong_edges.csv', 'w', encoding='utf-8')
    # csv_writer = csv.writer(f)
    # csv_writer.writerow(
    #     ["contract Sequence", "jump target(hex) ", "source", "destination","bytecode"])
    # f.close()
    #start testing
    all_fe=[]
    ...
    for i in range(len(test_bytecode)):
        ...
        #all_fe.append([i+1,search_fe])
        # with open('/home/jenny/PycharmProjects/ByteEye/Core/Analysis/wrong_edges.csv', mode="a",
        #           newline="",encoding='utf-8') as cfa:
        #     wf=csv.writer(cfa)
        #     for ev in AA.wrongInfo:
        #         wf.writerow(ev)
        cfg.cfg_output(AA, int(filename[i]),log_message,edge_analysis)
        # uniq=[]
        # for k in range(len(cfg.Basic_blocks)):
        #     if cfg.Basic_blocks[k].onlyAmode==False:
        #       uniq.append(k)
