...
def read_data():
    ...
    for count in range(541):
        ...
        map=a['cfg']
        maps.append(map)
    with open('Test Result/aaat0.json', 'r') as f:
        a = json.load(f)
...
def load_datasets():
    ...
    ############################################################################
    # new scp_reentrancy
    # data = xlrd.open_workbook('Core/Input/Dataset_SB/SB_new_label.xlsx')
    ...
    # from scp_integeroverflowdataset
    ...
    #esc_Timestamp check
    fp = open('Core/Input/scpt/timestamp_label.txt','r')
    for eachline in fp.readlines():
        lines = eachline.rstrip('\n')
        Labels.append(int(lines))
    fp.close()
    idx_train=106
    idx_valid=300
    idx_test=132
